...
// Next.js 에서 제공해주는 SSR, SSG 지원 타입 참조하기
...
export default function List({ result }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  // 테스트용으로 클라이언트에서 화면이  최초 렌더링될 때
  // Client data feetching을 이용해 백엔드 API에서 데이터를 불러와 데이터바인딩 해본다.
  ...
  useEffect(() => {
    ...
    const getSingleBlog = async () => {
      const res = await fetch('http://localhost:3005/api/articles/1', {
        method: 'GET',
        headers: {
...
// 해당 Client Component에서 SSR(Server Side Rendering을 처리해주는 비동기 함수(NEXT.js))
